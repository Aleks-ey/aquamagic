...
import {
  ...
  insertMedicalInfo,
  insertSchedule,
} from "../utils/dbInserts";
import {
  updateRegistrant,
  updateEmergencyContacts,
  updateMedicalInfo,
  updateSchedule,
} from "../utils/dbUpdates";
...
export default function Registration() {
  ...
  const [showExistingRegistrantPopup, setShowExistingRegistrantPopup] =
    useState(false);
  ...
  async function handleUpdateExistingRegistrant() {
    try {
      ...
      const registrantError = await updateRegistrant(
        formData,
        existingRegistrant.registrant_id,
      );
      if (registrantError) throw registrantError;

      // Update emergency contacts
      const contactErrors = await updateEmergencyContacts(
        formData,
        existingRegistrant.registrant_id,
      );
      ...
      const medicalError = await updateMedicalInfo(
        formData,
        existingRegistrant.registrant_id,
      );
      ...
      // Update schedule
      const scheduleError = await updateSchedule(
        formData,
        existingRegistrant.registrant_id,
      );
  ...
  async function handleInsertRegistrant() {
    try {
      ...
      if (authedUser) {
        await supabase.from("swimmer_accounts").insert({
          user_id: authedUser.id,
          registrant_id: registrantId,
        });

        // ignore "duplicate" errors (in case they re-register same swimmer)
        if (
          linkError &&
          !String(linkError.message).toLowerCase().includes("duplicate")
        ) {
          console.error("Error linking swimmer to account:", linkError);
        }
      }

      // Insert emergency contacts
      const contactErrors = await insertEmergencyContacts(
        formData,
        registrantId,
      );
      if (contactErrors.length > 0) throw contactErrors;
        ...
        <div className="bg-white p-5 rounded-md text-center">
          {status === "success" && (
            <p>
              ...
              <br />
              Please wait to be contacted by our team.
            </p>
          )}
          {status === "error" && (
  ...
  return (
    <div className="mt-32 mb-10 flex flex-col justify-center text-center">
      ...
      <form
        ...
        className="mt-10 md:mt-20 flex flex-col justify-center w-full px-5 mx-auto md:w-1/2 md:px-0"
      >
        {/* Registrant fields */}
        <table>
          <thead>
          ...
          <tbody>
            <tr>
              <td className="text-left w-1/4 pr-4">First Name:</td>
              <td>
                <input
            ...
            <tr>
              <td className="text-left pr-4">Last Name:</td>
              <td>
                <input
                  className="w-full px-2 border-gray-400 border-2 rounded-sm"
                  type="text"
            ...
            <tr>
              <td className="text-left pr-4">Date of Birth:</td>
              <td>
                <input
                  className="w-full px-2 border-gray-400 border-2 rounded-sm"
                  type="date"
            ...
            <tr>
              <td className="text-left pr-4">Phone:</td>
              <td>
                <input
                  className="w-full px-2 border-gray-400 border-2 rounded-sm"
                  type="text"
            ...
            <tr>
              <td className="text-left pr-4">Email:</td>
              <td>
                <input
                  className="w-full px-2 border-gray-400 border-2 rounded-sm"
                  type="email"
            ...
            <tr>
              <td className="text-left pr-4">Home Address:</td>
              <td>
                <input
                  className="w-full px-2 border-gray-400 border-2 rounded-sm"
                  type="text"
        ...
        {formData.guardians.map((guardian, index) => (
          <table key={index}>
            <thead>
              <tr>
                <th colSpan="2">
                  <h4 className="mt-10 text-left text-large font-semibold">
                    Guardian {index + 1}
                  </h4>
            ...
            <tbody>
              ...
              <tr>
                <td className="text-left pr-4">Relation:</td>
                <td>
                  <input
                    className="w-full px-2 border-gray-400 border-2 rounded-sm"
                    type="text"
        ...
        <table className="medical-info">
          {[
            ...
            "mental_problems",
            "conditions_after_injury_or_surgery",
            "depression/anxiety",
            "behavioral_problems",
            "medication",
            "surgery",
            ...
            <tbody key={field} className="field w-full">
              <tr className="w-full">
                <td className="text-left w-1/2">-{field.replace(/_/g, " ")}</td>
        ...
        <table className="medical-info">
          {[
            "observations_from_birth_to_1_YO",
            "observations_from_2_to_3_YO",
            "observations_from_4_to_7_YO",
            "observations_from_8_to_12_YO",
            "observations_from_13_to_16_YO",
            "observations_17_and_up",
          ].map((field) => (
            <tbody key={field} className="field w-full">
              <tr className="w-full">
                <td className="text-left w-1/2">-{field.replace(/_/g, " ")}</td>
        ...
        <table>
          ...
          <tbody>
            <tr>
              <td className="text-left w-1/4 pr-4">Monday:</td>
              <td className="pb-5">
                <input
                  className="w-full px-2 border-gray-400 border-2 rounded-sm"
                  type="text"
            ...
            <tr>
              <td className="text-left pr-4">Tuesday:</td>
              <td className="pb-5">
                <input
                  className="w-full px-2 border-gray-400 border-2 rounded-sm"
                  type="text"
            ...
            <tr>
              <td className="text-left pr-4">Wednesday:</td>
              <td className="pb-5">
                <input
                  className="w-full px-2 border-gray-400 border-2 rounded-sm"
                  type="text"
            ...
            <tr>
              <td className="text-left pr-4">Thursday:</td>
              <td className="pb-5">
                <input
                  className="w-full px-2 border-gray-400 border-2 rounded-sm"
                  type="text"
            ...
            <tr>
              <td className="text-left pr-4">Friday:</td>
              <td className="pb-5">
                <input
                  className="w-full px-2 border-gray-400 border-2 rounded-sm"
                  type="text"
            ...
            <tr>
              <td className="text-left pr-4">Saturday:</td>
              <td className="pb-5">
                <input
                  className="w-full px-2 border-gray-400 border-2 rounded-sm"
                  type="text"
            ...
            <tr>
              <td className="text-left pr-4">Sunday:</td>
              <td className="pb-5">
                <input
                  className="w-full px-2 border-gray-400 border-2 rounded-sm"
                  type="text"
        ...
        <div className="flex justify-center text-center">
          <button
            type="submit"
            className="w-48 mt-16 p-2 border-pool-water border-2 rounded-md text-pool-water hover:text-white hover:bg-pool-water"
          >
            Submit
          </button>
        </div>
      ...
      {submissionStatus && (
        <ConfirmationPopup status={submissionStatus} onClose={handleClose} />
      )}
      {showExistingRegistrantPopup && (
        <ExistingRegistrantPopup
          registrant={existingRegistrant}
          onClose={handleClose}
          onUpdate={handleUpdateExistingRegistrant}
        />
      )}
    </div>
  );
